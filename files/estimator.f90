...
!
   ...
   integer(kind=i4) :: i
   real(kind=r8) :: valn,val,err,val1,val2,err1,err2,error
   
   
   do i=1,nest
      call result(i,valn,val,err)
...
! error for function like A/B, given delta A anad delta B.   val1=A,val2=B       
       ...
       if (myrank().eq.0)  then      
         open(unit=13,FILE='energy_evolution.txt',FORM='FORMATTED',position='append') 
          write (13,'(g15.7,'' +-'',g15.7)') val1/val2,error  
         close(13)
       endif
