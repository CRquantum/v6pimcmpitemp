...
! perhaps make those constants a type, which can be used in all the module.
   ...
   real(kind=r8) :: rbisect,rmov1,rmov2,rmov3,rmov23 
    ...
    if (myrank().eq.0) then
        ...
        read (5,'(a70)') infile
        read (5,'(a70)') outfile
        ! for mpi, need to stike an 'Enter' to make one more line in the unit 5 file.
...
! protection check begin:
        if (myrank().eq.0) then   
            ...
            if ((nchorizo <= 2).or.(mmax <= 2)) then
	            write(6,*) 'small nchorizo and/or mmax, no need for PIMC, try VMC. Stop!',nchorizo,mmax
	            call abort                    
            endif
! protection check finished.            
            ...
            write (12,'(''bisection (reptation) upper bar='',t40,f10.5)') rbisect 
            write (12,'(''move 1 by 1 upper bar='',t40,f10.5)') rmov1 
            write (12,'(''move all beads upper bar='',t40,f10.5)') rmov2 
            write (12,'(''shift beads upper bar='',t40,f10.5)') rmov3 
            write (12,'(''move all + shift beads upper bar='',t40,f10.5)') rmov23 
            if (isite) then
        ...
        call stepinit(npart,nchorizo,hbar,dt,mmax,nrepmax,irep,nstepdecor,x0step,mov1step,mov2step,shftstep,nrhobin &
            ,icorrchk,nprot,nav,nstep,neq,rbisect,rmov1,rmov2,rmov3,rmov23)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   
    ...
    call stepinit(npart,nchorizo,hbar,dt,mmax,nrepmax,irep,nstepdecor,x0step,mov1step,mov2step,shftstep &
        ,nrhobin,icorrchk,nprot,nav,nstep,neq,rbisect,rmov1,rmov2,rmov3,rmov23)
    call brutinit(npart,nprot,hbar,iem) ! 3 in av18pot.f is v6' . this gives the initial L,R order for psi_T.
    !call stepchorizoinit(isite,infile,outfile)   ! call brutinit first.	
