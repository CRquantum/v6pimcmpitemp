...
c -------------
      ...
      mn=939.56563d0*xmn
      v7cut=.false.
      if (lpotin.eq.25) then
         lpotin=10 ! reset everything as for AV8'
         v7cut=.true.
      endif
      lpot=lpotin
      if (lpot.ge.4 .and. lpot.le.6) h2m=197.33d0**2/(938.9d0*xmn)
...
c ----------------------------------------------------------------------
      subroutine empotmod(lemp,xmn,ems,rr,vem)
      implicit real*8 (a-h,o-z)
      implicit integer (i-n)
      dimension vem(14)
