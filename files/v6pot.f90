module v6pot
    ...
    integer(kind=i4), private, save :: npart,ntab,ibox,ntabrsmall,ntabrbig
    real(kind=r8), private, allocatable, save, dimension(:,:,:) :: vtab,vtabls
    real(kind=r8), private, allocatable, save, dimension(:,:) :: vtabrsmall,vtabrbig
    real(kind=r8), private, allocatable, save, dimension(:) :: vemtab
    real(kind=r8), private, save :: scale,range,el,eli,vcbox,rangevrsmall,rangevrbig
...
contains
    subroutine v6potinit(npartin,iboxin,elin,lpot,ntabin,vfact,lpotpr,iem &
                        ,ntabrsmallin,ntabrbigin,rangevrsmallin,rangevrbigin)
    use cheft
    !use mympi
    real(kind=r8) :: vfact(8),elin,rangevrsmallin,rangevrbigin
    integer(kind=i4) :: ntabin,lpot,i,npartin,iboxin,lpotpr,ntabrsmallin,ntabrbigin
    real(kind=r8) :: dr,vv(18),vem(14),drsmall,drbig
    real(kind=r8) :: rr
    real(kind=r8) :: v0r,v0s,v0t,vr,vs,vt,kr,ks,kt
    real(kind=r8), parameter :: verytiny=1.0d-10
    logical :: iem
    ibox=iboxin
    npart=npartin
    el=elin
    ...
    ntabrsmall=ntabrsmallin
    ntabrbig=ntabrbigin
    rangevrsmall=rangevrsmallin
    rangevrbig=rangevrbigin
      
    drsmall=rangevrsmall/ntabrsmall
    drbig=(rangevrbig-range)/ntabrbig
      
    if (.not.allocated(vtabrsmall)) allocate(vtabrsmall(8,0:ntabrsmall))
    if (.not.allocated(vtabrbig)) allocate(vtabrbig(8,0:ntabrbig))     
    ...
    if (abs(lpot).lt.26) then
        ...
        do i=0,ntab
            rr=i*dr
            !call pot(0,rr,vv,vp,ww)
		    call av18op(lpot,rr,vv) ! 3 means v6' .
		    !vv3=abs(vv-vv2)
    ...
    !        endif 		 
        ...
        do i=0,ntabrsmall
            rr=i*drsmall
		    call av18op(lpot,rr,vv) ! 3 means v6' here.
            vtabrsmall(1:8,i)=vfact(1:8)*vv(1:8)
        enddo    
        do i=0,ntabrbig
            rr=rangevrbig+i*drbig
		    call av18op(lpot,rr,vv) ! 3 means v6' here.
            vtabrbig(1:8,i)=vfact(1:8)*vv(1:8)               
        enddo
    ...
    ! Minnesota potential:
        ...
        enddo
    elseif (abs(lpot).gt.100) then ! cheft
    ...
    subroutine getvtabls(vout) ! call v6potinit first
    integer(kind=i4) :: i
    real(kind=r8) :: vout(2,0:ntab)
    do i=0,ntab
	    vout(:,i)=vtabls(1,:,i)      
    enddo          
    return
    end subroutine getvtabls
   
    subroutine getvtabschmidt(vout1,vrsmallout,vrbigout) ! call v6potinit first, Schmidt order. use this to linearize e^(v6).
    integer(kind=i4) :: i,myunit
    real(kind=r8) :: vout1(6,0:ntab),vrsmallout(6,0:ntab),vrbigout(6,0:ntab)
    ...
    enddo
    do i=0,ntabrsmall
        vrsmallout(1,i)=vtabrsmall(1,i)  
        vrsmallout(2,i)=vtabrsmall(3,i) 
        vrsmallout(3,i)=vtabrsmall(5,i)  
        vrsmallout(4,i)=vtabrsmall(2,i) 
        vrsmallout(5,i)=vtabrsmall(4,i) 
        vrsmallout(6,i)=vtabrsmall(6,i)  
    enddo
    do i=0,ntabrbig
        vrbigout(1,i)=vtabrbig(1,i)  
        vrbigout(2,i)=vtabrbig(3,i) 
        vrbigout(3,i)=vtabrbig(5,i) 
        vrbigout(4,i)=vtabrbig(2,i) 
        vrbigout(5,i)=vtabrbig(4,i) 
        vrbigout(6,i)=vtabrbig(6,i) 
    enddo

    return
    end subroutine getvtabschmidt
   
    subroutine v6schmidtord(vv,v) ! call v6potinit first, Schmidt order. use this to linearize e^(v6).
    real(kind=r8) :: vv(18),v(6)
    v(1)=vv(1)
    v(2)=vv(3)
    v(3)=vv(5)
    v(4)=vv(2)
