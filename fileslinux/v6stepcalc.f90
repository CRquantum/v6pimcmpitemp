...
contains	   
    ...
    subroutine op3a(r,i,j,nstate,feff,cwtnew) ! op3a = tensor witout op2 and without factor 3.
    integer(kind=i4) :: i,j,nstate,spn,newspn,newspni,newspnj,signi,signj
    real(kind=r8) :: r(3)
    complex(kind=r8) :: cwtnew(0:nspin-1,nisospin),feff
    ...
    newspn=xor(newspni,shiftl(1,j-1))
    !write(6,*) r
    cwtnew(newspn,invispin(nstate))=feff*( r(1)**2 &	! six sjx
	                                -r(2)*((signi*signj)*r(2) & ! siy sjy
	                                       -(signi+signj)*ci*r(1))  )   & 
	                                +cwtnew(newspn,invispin(nstate))
    cwtnew(newspni,invispin(nstate))=feff*(signj*r(3)*(r(1)+signi*ci*r(2)))       & 
								    +cwtnew(newspni,invispin(nstate))
    cwtnew(newspnj,invispin(nstate))=feff*(signi*r(3)*(r(1)+signj*ci*r(2))) &
	                                +cwtnew(newspnj,invispin(nstate))
    cwtnew(spn,invispin(nstate))=feff*signi*signj*r(3)**2+cwtnew(spn,invispin(nstate)) ! siz sjz
    return
    end subroutine op3a    
    ...
    integer(kind=i4) :: i,j,iv,jv,nstate,nstatenew,ispn,newispn,spn,newspn
    spn=invspin(nstate) ! spn, 0:15.
    iv=shiftr(and(spn,shiftl(1,i-1)),i-1)
    jv=shiftr(and(spn,shiftl(1,j-1)),j-1)
    newspn=spn+(jv-iv)*2**(i-1)+(iv-jv)*2**(j-1)
    ...
    subroutine v6prop1(x,i,j,u,cwtold,cwtnew) ! optimized version 20190703, 25 times faster than old verison.
    ...
    do nstate=1,nbasis
        ...
        cwtnew(invspin(nstatenew1),invispin(nstatenew1))=cwtnew(invspin(nstatenew1),invispin(nstatenew1)) &
                                                    +fsex
        ! fiex,opextau        
        call opextau(i,j,nstate,nstatenew2)
        cwtnew(invspin(nstatenew2),invispin(nstatenew2))=cwtnew(invspin(nstatenew2),invispin(nstatenew2)) &
                                                    +fiex        
        ! fisex
        ...
        !call opextau(i,j,nstate,nstatenew)
        call op3a(r,i,j,nstatenew2,fteffiex,cwtnew)
